# === ENHANCED POC - Multi-Level Comments with Tree Structure ===
...
class DocxMarkdownParser:
    ...
    def _add_threaded_comments(self, markdown_content, comments, authors, dates, 
        ...
        def format_comment(comment_id, level=0):
            ...
            output = f"{indent}### {prefix}Comment [{comment_id}]"
            
            if level > 0:
                parent_id = thread_dict.get(comment_id)
                output += f" ↳ Reply to [{parent_id}]"
            ...
            if level == 0 and comment_id in commented_text and commented_text[comment_id].strip():
                output += f"{indent}**Referenced Text**: \"{commented_text[comment_id]}\"\n\n"
